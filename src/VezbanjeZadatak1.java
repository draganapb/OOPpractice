public class VezbanjeZadatak1 {
    public static void main(String[] args) {
        //Zadatak 1:
        //Kreirate vas profil
        //Program mora da sadrzi ime, prezime, godine i boolean vrednost da li ucite Javu
        //Promenljive odstampati u konzoli
        //Dodatak: Kroz char ubaciti i pol

        String ime = "Dragana";
        String prezime = "Topic";
        int godine = 32;
        boolean java = true;
        char pol = 'z';

        System.out.println("Moj profil:");
        System.out.println("Ime:" + ime);
        System.out.println("Prezime" + prezime);
        System.out.println("Godine: " + godine);
        System.out.println("Ucite li javu?" + java);
        System.out.println("Unesite pol" + pol);


















    }
}
